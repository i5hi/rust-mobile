...
#[no_mangle]
pub extern fn seed_to_master_xprv(mnemonic:  *const c_char, passphrase:  *const c_char, network:  *const c_char) -> *mut c_char  {
...
#[no_mangle]
pub extern fn derive_hardened(master_xprv: *const c_char, account: *const c_char) -> *mut c_char {
    ...
    let result = fingerprint.to_string()+":"+&hardened_path+":"+&child_xprv.to_string()+":"+&child_xpub.to_string();
    CString::new(result).unwrap().into_raw()

}

#[no_mangle]
pub extern fn sign_solo_psbt(
    fingerprint: *const c_char, 
    account_index: *const c_char, 
    account_xprv: *const c_char, 
    psbt: *const c_char) -> *mut c_char 
{
    ...
    let key_descriptor = String::from("[") +fingerprint+ "/84h" + network_path + "/" + account_index +"h]" + &account_xprv.to_string() ;

    let mut desc_deposit = String::from("wpkh(") + &key_descriptor + "/0/*)";
    let mut desc_change =  String::from("wpkh(") + &key_descriptor + "/1/*)";
    desc_deposit = desc_deposit.clone() + "#" + &checksum::get_checksum(&desc_deposit.clone()).unwrap();
    desc_change = desc_change.clone() + "#" + &checksum::get_checksum(&desc_change.clone()).unwrap();
    ...
    let psbt_struct:PartiallySignedTransaction = deserialize(&base64::decode(psbt).unwrap()).unwrap();
...
mod tests {
    ...
    fn test_bitcoin_composite() {
        ...
        let expected = String::from(fingerprint)+":"+ hardened_path + ":" + account_xprv + ":" + account_xpub;
